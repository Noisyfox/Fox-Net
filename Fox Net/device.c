...
#pragma NDIS_INIT_FUNCTION(FoxNetRegisterDevice)
    ...
    )
{
    ...
    DEBUGP(DL_TRACE, "==>FoxNetRegisterDevice\n");
   
    
    NdisZeroMemory(DispatchTable, (IRP_MJ_MAXIMUM_FUNCTION+1) * sizeof(PDRIVER_DISPATCH));
    
    DispatchTable[IRP_MJ_CREATE] = FoxNetDispatch;
    DispatchTable[IRP_MJ_CLEANUP] = FoxNetDispatch;
    DispatchTable[IRP_MJ_CLOSE] = FoxNetDispatch;
    DispatchTable[IRP_MJ_DEVICE_CONTROL] = FoxNetDeviceIoControl;
    ...
    DEBUGP(DL_TRACE, "<==FoxNetRegisterDevice: %x\n", Status);
